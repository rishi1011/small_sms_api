...
def get_school_years(*, db: Session = Depends(get_db), skip: int = 0, limit: int = 20):
    return service.get_school_years(db, skip=skip, limit=limit)


@router.get("/{school_year_id}", response_model=SchoolYearInDB)
def get_school_year(*, db: Session = Depends(get_db), school_year_id: int):
    return service.get_school_year(db, school_year_id=school_year_id)


@router.post("", status_code=status.HTTP_201_CREATED)
def create_school_year(*, db: Session = Depends(get_db), school_year_in: SchoolYearCreate):
...
def activate_school_year(*, db: Session = Depends(get_db), school_year_id: int):
...
def delete_school_year(*, db: Session = Depends(get_db), school_year_id: int):
